...
**Reconciliation with `CORRECTIONS.md` (2026-08-08).** CORRECTIONS.md is the
running readability log, and several of its entries are owner rulings dated
*after* this document's v6 codification (2026-07-27). Where the two differ on
readability, formatting, or tradeoff count, the dated CORRECTIONS ruling is the
current law and supersedes the older line here. Folded in below: the fixed
"4–6" / "six-standard" tradeoff count is replaced by "list every genuine
tradeoff, no target count" (owner ruling 2026-07-31); the "long-sentence voice
stays" line is qualified by the ~40-word sentence ceiling (owner ruling
2026-08-05, P28); the cruxSummary target is aligned to the codified 12–16 band.
Tracked in CORRECTIONS but deliberately NOT folded in here pending an owner
...
List every genuine tradeoff the source earns — there is no target count (it
could be two, it could be eight); the only tests are whether each is a real
cost the source supports and whether any genuine one is missing (owner ruling
2026-07-31). Each is a **full paragraph**, never a one-liner. Each names: the
...
- **Glossing is the fix; deletion is not.** Technical precision and
  ...
  **[Qualified 2026-08-05 — owner ruling P28: sentence LENGTH is now
  capped at ~40 words (soft ceiling; 50+ almost always splits, 60+
  must break). Splitting a long sentence is neither deletion nor a
  loss of precision — it barely changes the char count — so it is the
  same no-substance-removed spirit applied to length. Density of
  thought stays; run-on length does not. See `CORRECTIONS.md` P28.]**
...
carries a single-line **cruxSummary** — 12–16 words (the codified v6 band) compressing the crux
...
> solution / every genuine tradeoff, no target count / 2–6 general patterns) in a restrained,
...
- **tradeoffs:** SIX is the standard (19 of 25 live articles);
  ...
  **[SUPERSEDED 2026-07-31 — owner ruling: drop the target count
  entirely. List every genuine tradeoff the source earns, with no
  fixed number (could be two, could be eight); the test is whether
  each is a real cost and whether any genuine one is missing. The
  corpus ratio above is history, not a target. See `CORRECTIONS.md`.]**
