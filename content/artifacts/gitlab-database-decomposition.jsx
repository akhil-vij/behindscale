...
export default function HalfTheWrites() {
  ...
  const verdict = (() => {
    ...
    if (w.last === "zerodt" && w.zeroDeclined) return { c: VIOLET, code: "ROLLBACK BEAT ZERO DOWNTIME", t: "The near-zero plan existed in full: pause CI writes, note how far the main database had gotten, wait for the CI copy to catch up, promote, and switch. Declined on three stated grounds - no easy rollback (CI writes landed mid-migration would be lost), a few-seconds error window that muddies the did-it-work signal, and no hard business requirement for zero. Downtime is a cost; rollback clarity is a benefit; they did the arithmetic out loud. Schedule the window instead." };
    ...
    if (w.stage === "prep") {
      ...
      if (w.last === "lfk") return { c: VIOLET, code: "CASCADE BY QUEUE, NOT BY LOCK", t: "Foreign keys across the line are gone. In their place, a Postgres trigger fires on every delete (even bulk deletes, which some app-level code misses) and writes the parent deletion to a queue that a background job drains in controlled batches. Same delete or set-to-null behavior, just a moment later. The dividend: the giant cascading deletes that used to time out now proceed at whatever pace the database can afford." };
    ...
    if (w.last === "replica") return { c: AMBER, code: "REPLICAS DON'T WRITE", t: `${w.replicas} read-only replicas now serve reads - and the write path is exactly as tall as before, because every write must reach the one primary. Read scaling is incremental and solved; the write ceiling is structural. This is why the pools 'only got us so far.'` };
    if (w.last === "buyvm") return { c: RED, code: "NO BIGGER MACHINE EXISTS", t: "The primary already runs on 96 CPU cores, the limits of a single machine, and 'continually trying to vertically scale this VM would eventually not be possible.' Even a hypothetically infinite machine leaves a 22TiB database that grows harder to manage and keeps causing incidents. The ladder ends here." };
    if (w.last === "shard") return { c: VIOLET, code: "THE LONG ROAD, DEFERRED", t: "Spreading the data across servers by account (the top-level namespace) was explored first, but the app was never built to keep each account's data cleanly separate, so the problems piled up. The judgment is precise: ultimately this is a good way to split and scale, but the scaling problem needs a shorter-term answer. The long-term move stays on the map; the reachable one goes first." };
    ...
    return { c: AMBER, code: "ONE PRIMARY, 22 TERABYTES, EVERY WRITE", t: "A well-scaled single database: read-only copies and connection pooling in front - and a single main server with 96 cores that every write must reach. Try the classic moves first; then RUN THE WRITE ANALYSIS and find the split line the data chooses." };
  })();
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#ffd0b3" : "#8b90a0", background: on ? "rgba(252,109,38,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>You run the yearlong split: find the write limit, pick which half to move out, keep new problems from creeping back, and switch over in a window you have rehearsed.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.btn(false, !mono_)} disabled={!mono_} onClick={D("buyvm")}>BUY A BIGGER PRIMARY (&gt;96 cores)</button>
          ...
          <button style={{ ...S.btn(false, !prep), border: `1px solid ${RED}`, background: prep ? `${RED}14` : "#0c0d13", color: prep ? "#ffc9c9" : "#8b90a0" }} disabled={!prep} onClick={D("newjoin")}>SOMEONE MERGES A NEW CROSS-JOIN</button>
        ...
        <div style={{ flex: "2 1 440px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
              <div style={{ flex: 1, minWidth: 110 }}><div style={S.label}>PRIMARY</div><div style={{ fontSize: 14, fontWeight: 700, color: w.stage === "results" ? GREEN : RED }}>{w.stage === "results" ? "2 hosts · headroom" : "96/96 cores"}</div><div style={{ fontSize: 9, color: "#6b7080" }}>{w.stage === "results" ? "writes split ~in half" : "every write lands here"}</div></div>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>GitLab.com's 22TiB Postgres database sat behind read-only copies that scale reads but can't touch the real ceiling: every write reaches one main server, that server is already the biggest machine available (96 cores), and no larger one exists. Postgres's background cleanup of dead rows ran flat out at 80-100%, and background connections were held back to avoid overloading it.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Split the database in half by write traffic - CI tables carried ~49% of writes - kept clean by a gate that blocks any new cross-database query, loose foreign keys to keep cross-database deletes working, a rollout that made the app act as if it had two databases so the final step was tiny, and a deliberately declined zero-downtime plan in favor of a rehearsed two-hour window that ran in 93 minutes.</div>
