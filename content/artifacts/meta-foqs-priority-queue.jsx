...
export default function ShockAbsorber() {
  ...
  const crashConsumer = () => { w.current.leases.push({ at: w.current.t + 2 }); force((x) => x + 1); };
  ...
  const verdict = (() => {
    ...
    if (W.leases && W.leases.length > 0) return { c: ACCENT, code: "LEASE RUNNING - THE CLOCK DECIDES", t: `A consumer dequeued an item and then crashed before it could ack. The item is not lost and not yet redelivered - its lease is counting down (${W.leases.length} in flight). When the lease expires with no ack, FOQS ${atMostOnce ? "deletes the item: at-most-once means no duplicate, at the cost of that one item" : "makes it deliverable again: at-least-once means no loss, at the cost of a possible duplicate"}. Watch REDELIVERED / ITEMS LOST tick when the clock runs out.` };
    if (W.redelivered > 0 || W.lost > 0) return atMostOnce
      ...
      : { c: GREEN, code: "LEASE EXPIRED - REDELIVERED, NOT LOST", t: `A consumer crashed after dequeue and never acked; the lease ran out and FOQS made the item deliverable again (${W.redelivered} redelivered, ${Math.round(W.lost)} lost). This is the queue's guaranteed-delivery property - the one Discord reached for in PubSub - and its tax: a merely slow consumer gets duplicates, so idempotent processing is every consumer's obligation.` };
  ...
  const S = {
    ...
    btn: (on) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: "pointer", border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#a8c7ff" : "#9aa0b0", background: on ? "rgba(8,102,255,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-08-06 review): the pivotal toggles (outage, shard-down, checkpointing, semantics)
    // carry a permanent signal-color border and show state by fill, never by brightness, so an off
    // toggle stays findable against the dark background. Momentary buttons use the lightened btn border.
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 240px", minWidth: 240 }}>
          ...
          <button style={S.tog(outage, "#EF4444")} onClick={() => setOutage(!outage)}>💥 DOWNSTREAM OUTAGE {outage ? "· ON" : "· OFF"}<div style={{ color: "#8b90a0", fontSize: 10 }}>every consumer stops - the backlog is the point</div></button>
          <button style={S.btn(false)} onClick={crashConsumer}>💥 CRASH A CONSUMER MID-LEASE<div style={{ color: "#8b90a0", fontSize: 10 }}>a consumer took an item but died before confirming it - watch its lease count down, then resolve</div></button>
          <button style={S.tog(shardDown, "#eab308")} onClick={() => setShardDown(!shardDown)}>💥 SHARD 3 GOES SLOW {shardDown ? "· ON" : "· OFF"}<div style={{ color: "#8b90a0", fontSize: 10 }}>rolling-window health trips the circuit breaker</div></button>
          ...
          <button style={S.tog(checkpoint, ACCENT)} onClick={() => setCheckpoint(!checkpoint)}>CHECKPOINTING: {checkpoint ? "ON" : "OFF"}<div style={{ color: "#8b90a0", fontSize: 10 }}>bound timestamp scans from both sides</div></button>
          <button style={S.tog(atMostOnce, "#84CC16")} onClick={() => setAtMostOnce(!atMostOnce)}>SEMANTICS: {atMostOnce ? "AT MOST ONCE" : "AT LEAST ONCE"}<div style={{ color: "#8b90a0", fontSize: 10 }}>lease expiry: redeliver, or delete</div></button>
        ...
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ flex: "1 1 130px", background: "#0c0d13", border: `1px solid ${(W.leases && W.leases.length) ? ACCENT : "#2a2a3a"}`, borderRadius: 6, padding: "8px 10px" }}>
                <div style={{ fontSize: 9, letterSpacing: 1.5, color: (W.leases && W.leases.length) ? ACCENT : "#6b7080" }}>LEASES RUNNING</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: (W.leases && W.leases.length) ? ACCENT : "#c8cdd8" }}>{W.leases ? W.leases.length : 0}</div>
                <div style={{ fontSize: 9, color: "#6b7080", marginTop: 2 }}>{W.redelivered} redelivered</div>
              </div>
            </div>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Build the buffer so that absorbing the shock doesn't slow it down. Bound how much old history each database scan has to read. Combine the separate per-shard lists into one order so the most urgent item still comes out first. Hand each item to a consumer on a short lease, so if that consumer crashes the item comes back for redelivery instead of being lost. And stop sending new work to a shard that has gone unhealthy.</div>
