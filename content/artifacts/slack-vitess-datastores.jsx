...
function step(w) {
  const n = { ...w };
  n.t++;
  n.whale = w.whale + 2; // the customer keeps growing regardless of your architecture
  return n;
}

export default function LargestShard() {
  const [w, setW] = useState(initial);
  useEffect(() => { const id = setInterval(() => setW(step), 900); return () => clearInterval(id); }, []);
  const us = utils(w);
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#ffc6d8" : "#8b90a0", background: on ? "rgba(224,30,90,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>Add shards and you serve more customers. But one customer's size still aims at a wall. You operate the fleet while your biggest customer keeps hiring.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        ...
        <div style={{ flex: "2 1 440px", minWidth: 300 }}>
          ...
          <div style={{ ...S.panel, marginBottom: 12 }}>
            <div style={S.label}>MIGRATION TO VITESS</div>
            <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
              {["1 · PROTOTYPE", "2 · DOUBLE-WRITE + DIFF", "3 · RESHARD"].map((lab, i) => {
                const done = w.stage >= i + 1; const active = w.stage === i + 1;
                return (<div key={i} style={{ flex: 1, padding: "5px 6px", borderRadius: 5, textAlign: "center", fontSize: 9, border: `1px solid ${done ? VIOLET : "#4a4f60"}`, background: done ? "rgba(155,140,240,0.14)" : "#0c0d13", color: done ? "#d7cffb" : "#6b7080", fontWeight: active ? 700 : 400 }}>{lab}</div>);
              })}
            </div>
            <div style={{ ...S.label, marginTop: 6 }}>{w.stage === 0 ? "not started · the whale still sits on one shard" : w.stage === 3 ? "resharded by channel id · data now spread across the fleet" : "in progress · the data has not moved yet, so the whale is still hot"}</div>
          </div>
          <div style={S.panel}>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Change which layer owns placement: three years migrating 99% of query load to Vitess, where sharding is the datastore's job, behind a query layer that hides where data lives, so message data could reshard by channel id, dissolving the whale across the fleet, without the application knowing anything moved.</div>
