...
const CAP = 100; const BASE = 40; const PEAK = 120; const STEP_DROP = 28;
...
function step(w) {
  ...
  if (n.dbLoad <= CAP) { n.hit = Math.min(100, n.hit + 3); n.over = 0; }
  else { n.hit = Math.max(5, n.hit - 4); n.over = n.dbLoad - CAP; } // timeouts: the refill queries are the ones dying
  n.retryLoad = Math.min(80, Math.round(n.over * 0.4));
  return n;
}

export default function TwoTwentyTwo() {
  const [w, setW] = useState(initial);
  useEffect(() => { const id = setInterval(() => setW(step), 850); return () => clearInterval(id); }, []);
  ...
  const verdict = (() => {
    ...
    if (w.throttled && w.hit >= 95) return { c: GREEN, code: "WARM AGAIN - WALK THE LIMIT HOME", t: "Caches full, database healthy. Keep raising in small steps until the throttle is gone and full traffic is restored - the real recovery walked the boot limit up while keeping useful database throughput high the whole way." };
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#b9f0d5" : "#8b90a0", background: on ? "rgba(46,182,125,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Break the loop from outside: throttle new logins below the tipping point (protecting already-connected users first), fix the query to fetch only what's missing and read from replicas, then walk the limit back up in small steps - because one too-large step caused a relapse.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Roll two upgrade steps off-peak and watch them get absorbed. Land the third at peak, cross the tipping point, then pause the rollout and watch it change nothing: the outage now keeps itself going. Throttle new logins to get back under the tipping point, raise the limit too far once to see it relapse, fix the expensive query, and walk the system home in small steps.</div>
    </div>
  );
}
