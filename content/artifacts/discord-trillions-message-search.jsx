...
const INJECTIONS = [
  ...
  { id: "bfg", label: "GUILD HITS 2 BILLION", hint: "one index reaches Lucene's two-billion limit" },
];
...
function verdictOf(era, injection, live) {
  ...
  if (injection === "log4") {
    ...
    return restartDone
      ...
      : V("ok", "ROLLING RESTART IN PROGRESS", `ECK restarts nodes one at a time - the amber sweep - while spare copies keep every index served. Success holding at ${success}%; dropped: ${Math.round(dropped)}.`);
  }
  // bfg
  if (era === "2017") return V("bad", "THE TWO-BILLION WALL", "Each Elasticsearch index is a single Lucene index underneath, and Lucene caps an index at about two billion documents. The biggest guilds hit it - every indexing operation for that index fails - and the recovery was finding large spammy guilds to delete. Not a strategy.");
  return V("ok", "A CELL FOR GIANTS", "Big guilds get a dedicated BFG cell whose indices run multiple primary shards, sidestepping the single-Lucene-index ceiling. Growing guilds migrate in via a six-step dual-index flow - new index, double writes, historical backfill, query switch, cleanup - with search up throughout. The cell design paying off.");
}

const PROBES = {
  guild: {
    "2017": { badge: "PRESERVED", text: "hash(the guild's ID) → index + cluster, computed in application code. All of a guild's messages live together, so one query touches one place." },
    "2025": { badge: "PRESERVED", text: "hash(the guild's ID) → index + cluster, the same application-code routing, now resolving into a cell. The one decision the rewrite kept." },
  },
  dm: {
    ...
    "2025": { badge: "EXTENDED", text: "DMs now index twice: by channel, and by user ID into the dedicated user-dm cell - so one query searches every DM you have. The preserved decision didn't just survive; it gained a dimension." },
  },
};
...
export default function BlastRadius() {
  ...
  const S = {
    ...
    btn: (on, disabled) => ({
      ...
      border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#c7ccff" : "#8b90a0",
            ...
            <div style={{ marginTop: 12 }}>
              {era === "2017" ? (
                <div style={{ background: "#0c0d13", border: "1px solid #2a2a3a", borderRadius: 6, padding: 10 }}>
                  ...
                  {injection === "bfg" && <div style={{ fontSize: 9.5, color: RED, marginTop: 6 }}>■ index on node 3: FULL - Lucene two-billion limit reached; all its indexing fails</div>}
                </div>
              ) : (
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>By trillions of messages, the failure modes all pointed at cluster bigness: one failed node in a 100-node cluster failed roughly 40% of bulk indexing operations, past 200 nodes the coordinating master node ran out of memory and cascaded, and rolling restarts got so slow the team ran unpatched versions until log4shell forced a full outage.</div>
