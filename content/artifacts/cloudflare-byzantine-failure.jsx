...
export default function NeitherDeadNorAlive() {
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#ffcf9e" : "#9aa0b0", background: on ? "rgba(246,130,31,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-07-31 review): the two fault toggles (kill dead / degrade) drive the whole
    // artifact, so they carry a permanent signal color - state shown by fill, never brightness,
    // so an off toggle stays findable instead of matching the dark background. col = the fault's color.
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.tog(sw === "dead", played, VIOLET)} disabled={played} onClick={() => { setSw("dead"); setPlayed(false); }}>KILL IT DEAD<div style={{ color: "#6b7080", fontSize: 10 }}>links drop - the case failover was built for</div></button>
          <button style={S.tog(sw === "degraded", played, RED)} disabled={played} onClick={() => { setSw("degraded"); setPlayed(false); }}>DEGRADE IT - HALF-ALIVE<div style={{ color: "#6b7080", fontSize: 10 }}>LACP/BGP up, data plane dropping packets</div></button>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>In 2020 a Cloudflare switch went half-alive - status protocols up, but quietly dropping some packets - and every failover layer built for up-or-down misfired: the coordination store's elections tied, and automation promoted a database primary that was not broken, forcing an hours-long rebuild of every standby copy. Six minutes of switch trouble became 6h33m of degraded API and dashboard.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Humans worked around the automation - shedding discretionary load and hand-steering reads to secondary-datacenter copies the failover process could not touch - and afterward Cloudflare slowed the promotion trigger: for an expensive cure, time is evidence.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Kill the switch dead first and watch failover handle it cleanly. Then degrade it, let the six minutes play out with the promotion trigger set low so it fires fast, and mitigate by hand - saving the API will cost the dashboard. Finish by setting the trigger past 6 and meeting the near miss.</div>
    </div>
  );
}
