...
export default function AtMostOnce() {
  ...
  const verdict = (() => {
    ...
    if (w.phase === "done") return { c: w.isRetry && w.doubles > 0 ? RED : GREEN, code: w.isRetry && w.doubles > 0 ? "RESPONSE DELIVERED - AND THE GUEST PAID TWICE" : "THE HAPPY PATH", t: w.isRetry && w.doubles > 0 ? "The retry completed cleanly from the client's view - success received, key consumed. The double charge already happened at the bank, invisible to a client that can only see responses." : "Charge sent, money moved once, response consumed. Uneventful - which is the whole point of the rules: the intent is recorded before the network call, the outcome after, and the network call never happens inside a database transaction. Now do it again and cut the response mid-flight." };
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#ffc2c4" : "#9aa0b0", background: on ? "rgba(255,90,95,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-08-05 review): ORPHEUS and READ SOURCE are the two toggles the whole sim pivots on,
    // so they carry a permanent signal-color border and show state by fill, never brightness, so an
    // off toggle stays findable instead of matching the dark background. col = the toggle's color.
    tog: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.45 : 1, border: `1px solid ${col}`, color: col, background: on ? `${col}29` : "#0c0d13", fontWeight: on ? 700 : 400, fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>One request, three phases, and a pair of scissors to cut the response. The bank always moves the money; the only thing in doubt is whether the client finds out.</p>
      <ContextBlock />

      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          <div style={S.label}>FRAMEWORK</div>
          <button style={S.tog(w.orpheus, !idle, ACCENT)} disabled={!idle} onClick={() => setW(x => ({ ...x, orpheus: !x.orpheus }))}>ORPHEUS: {w.orpheus ? "ON" : "OFF"}<div style={{ color: "#6b7080", fontSize: 10 }}>{w.orpheus ? "keys, leases, recorded responses" : "no keys - a retry is just a second request"}</div></button>
          <button style={S.tog(w.src === "R", !idle, "#eab308")} disabled={!idle} onClick={() => setW(x => ({ ...x, src: x.src === "M" ? "R" : "M" }))}>READ SOURCE: {w.src === "M" ? "MASTER" : `REPLICA (lag ${w.lag})`}<div style={{ color: "#6b7080", fontSize: 10 }}>{w.src === "M" ? "the framework sees its own writes" : "the framework sees the past"}</div></button>
        ...
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
              <div style={{ flex: 1, minWidth: 140, background: "#0c0d13", border: "1px solid #2a2f45", borderRadius: 6, padding: "8px 10px" }}><div style={S.label}>MASTER · a LEASED row = one attempt holds the key</div><div style={{ fontSize: 11, marginTop: 4 }}>{rows(w.master)}</div></div>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Orpheus, Airbnb's idempotency library: each request gets a key, that key locks a database row so only one attempt runs at a time (a lease), and once a request finishes its result is saved and replayed to any retry. All of this is read only from the main (master) database, because a lagging read copy would bring the double charge right back. Result: 99.999% payment consistency while volume doubled.</div>
