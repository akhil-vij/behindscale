...
const CAPS = [16, 4, 1]; // spike cycles capacity down; window follows - default (16) holds the whole 15-message lane, so aging appears only under load
const winDays = (cap) => (cap === 16 ? 28 : cap === 4 ? 7 : 0.8);
const initial = () => ({ t: 0, queue: ["m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14", "m15"], resent: [], ledger: [], aged: [], output: [], dedupeOn: false, tunnel: false, capIdx: 0, discarded: 0, dupDelivered: 0, crashed: false, diverged: null, lastEvent: null, recoveredAt: 0 });
...
export default function AlmostExactlyOnce() {
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ padding: "7px 10px", borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? (col || "#bfe8d6") : "#9aa0b0", background: on ? "rgba(82,189,148,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11, marginRight: 6, marginTop: 6 }),
    // FIX (2026-07-31 review): TUNNEL and LEDGER are the two state toggles the whole sim pivots on,
    ...
    tog: (on, col) => ({ padding: "7px 10px", borderRadius: 6, cursor: "pointer", border: `1px solid ${col}`, color: col, background: on ? `${col}29` : "#0c0d13", fontWeight: on ? 700 : 400, fontFamily: mono, fontSize: 11, marginRight: 6, marginTop: 6 }),
  ...
  return (
    <div style={S.root}>
      ...
      <div style={{ marginTop: 12 }}>
        <button style={S.tog(w.tunnel, AMBER)} onClick={() => setW(x => ({ ...x, tunnel: !x.tunnel }))}>🚌 TUNNEL: {w.tunnel ? "IN (responses dying)" : "clear"}</button>
        <button style={S.tog(w.dedupeOn, GREEN)} onClick={() => setW(x => ({ ...x, dedupeOn: !x.dedupeOn }))}>LEDGER: {w.dedupeOn ? "ON (has_seen?)" : "OFF"}</button>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Every message carries a client-generated ID. Kafka routes each ID to the same worker every time, and that worker's local RocksDB ledger answers "seen this?" within a four-week window that shrinks under load instead of failing. No single all-or-nothing step covers the ledger write, the publish, and the acknowledgment - so the output topic is treated as the source of truth, and everything else is repaired against it after a crash.</div>
