...
export default function ThePingComesFirst() {
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#ffd9a8" : "#9aa0b0", background: on ? "rgba(255,153,0,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-07-31 review): the fault toggles (surge/crawler/storm) drive every lesson, so
    // they carry their signal color permanently - state shown by fill, never brightness, so an
    // off toggle stays findable instead of matching the page background.
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.tog(w.surge, AMBER)} onClick={() => setW(x => ({ ...x, surge: !x.surge }))}>TRAFFIC SURGE: {w.surge ? "ON (+80 tps)" : "OFF"}</button>
          <button style={S.tog(w.crawler, AMBER)} onClick={() => setW(x => ({ ...x, crawler: !x.crawler }))}>CRAWLER WAVE: {w.crawler ? "ON (+60 tps, shiftable)" : "OFF"}</button>
          <button style={S.tog(w.storm, RED)} onClick={() => setW(x => ({ ...x, storm: !x.storm }))}>RETRY STORM: {w.storm ? "ON (timeouts retry ×1.4)" : "OFF"}</button>
