...
export default function WhenQueuesStopWorking() {
  ...
  useEffect(() => { const id = setInterval(() => setW(step), 950); return () => clearInterval(id); }, []);
  ...
  const verdict = (() => {
    ...
    if (w.topo === "dest") {
      ...
      if (w.poison === "retry") return { c: RED, code: "POISON PICKED: RETRY INTO THE LIMIT", t: `Keep sending past 1,000/s and the endpoint answers with a rate-limit rejection (429) (${w.r429} so far). The retries re-enter the queue and thicken the very block B is stuck behind. Work amplification wearing a delivery costume.` };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>Two things go wrong here - a partner outage and a whale customer's flood - run against Segment's three real designs. Customer A is the whale; GA is Google Analytics, the endpoint that fails.</p>
      <ContextBlock />
      ...
      <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button style={{ ...S.btn(w.outage, false), ...(w.outage ? { borderColor: RED, color: RED } : {}) }} onClick={() => setW(x => ({ ...x, outage: !x.outage, drainT: null }))}>PARTNER OUTAGE (GA): {w.outage ? "ON" : "OFF"}</button>
        <button style={{ ...S.btn(w.flood, false), ...(w.flood ? { borderColor: AMBER, color: AMBER } : {}) }} onClick={() => setW(x => ({ ...x, flood: !x.flood, poison: null }))}>WHALE FLOOD (A→GA): {w.flood ? "ON" : "OFF"}</button>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            ...
            <div style={{ marginTop: 6 }}>
              {w.topo !== "cent" ? (
                <>
                  {row("→ GA" + (w.outage ? " ⚠" : ""), w.backlog.GA, w.outage || (w.flood && w.topo === "dest") ? "bad" : "ok", w.topo === "dest" && w.flood ? "A A A A A … B  (50k contiguous)" : w.outage ? "server-error storm · 15% success" : "healthy")}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
              ...
              <div style={{ flex: 1 }}><div style={S.label}>RATE-LIMIT HITS</div><div style={{ fontSize: 15, fontWeight: 700, color: w.r429 ? RED : "#c8cdd8" }}>{w.r429}</div></div>
              <div style={{ flex: 1 }}><div style={S.label}>COPIED ASIDE</div><div style={{ fontSize: 15, fontWeight: 700, color: w.dlqBytes ? AMBER : "#c8cdd8" }}>{(w.dlqBytes / 1000).toFixed(1)}TB</div></div>
        ...
        <div style={{ ...S.panel, flex: "1 1 230px", minWidth: 230 }}>
          <div style={S.label}>PICK YOUR POISON {w.topo === "dest" && w.flood ? "· THE POST'S THREE EXITS" : "· (turn on PER-DESTINATION + WHALE FLOOD)"}</div>
          {poisonBtn("cap", "① HARD-CAP A AT 1,000/s - B waits 50s behind the block")}
          {poisonBtn("retry", "② KEEP SENDING - hit the rate limit again, retries thicken the block")}
          {poisonBtn("dlq", "③ COPY 49K MSGS TO A SIDE QUEUE - B flows, terabytes move")}
