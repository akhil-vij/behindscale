...
export default function WhenTheQueuePushesBack() {
  ...
  useEffect(() => { const id = setInterval(() => setW(step), 1100); return () => clearInterval(id); }, []);
  ...
  const verdict = (() => {
    ...
    if (rb && w.churn > 0) return { c: RED, code: "THE KILLS FEED THE LOAD", t: `Latency has crossed the timeout, so uWSGI is killing slow workers, thousands of them restart at once, and each restart re-opens a connection to the broker. Those reconnections are themselves load, so inflow is now demand PLUS ${Math.round(w.churn * 0.5)} tps of self-inflicted reconnection traffic. The slowdown is feeding itself; backlog ${Math.round(w.backlog)}.` };
    if (rb && w.flow) return { c: AMBER, code: "THE QUEUE PUSHED BACK", t: `Burst inflow crossed the broker's comfort line and consumption is degraded. Flow Control is throttling the fastest publishers, who feel it as network latency (${w.latency} units and climbing). At peak that latency cascades: requests pile up upstream, and once it crosses the timeout line, harakiri (a setting that kills any worker running too long) starts killing workers.` };
    if (w.mode === 1) return { c: AMBER, code: "HA MODE THAT TAXED THE HEADROOM", t: `HA (high-availability) mode keeps a backup copy of the broker so it can take over if the primary dies. But keeping that copy in sync cut throughput to ${cap} (from 100), and in practice the takeover took 20-plus minutes, often got stuck, and lost messages. Push the PEAK at it and watch the outage arrive sooner than on a single node.` };
    ...
    if (!rb) return { c: GREEN, code: "A DURABLE LOG UNDERNEATH", t: "Task names and pickled arguments on Kafka; a @task wrapper routing by feature flag so adopting teams changed one line. Push the PEAK at it, then meet head-of-line blocking and rebalance stalls - the new problems you chose over the ones you escaped." };
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#ffb8ad" : "#9aa0b0", background: on ? "rgba(235,23,0,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-08-06 review): pivotal toggles (peak, countdown, slow-message, non-blocking worker)
    // carry a permanent signal-color border and show state by fill, never brightness, so an off
    // toggle stays findable against the dark background. col = the toggle's signal color.
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>You operate the message broker - the server that holds the queue of tasks - under peak dinner rush. Its job is to absorb pressure; watch what it does with it instead.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.tog(w.peak, false, AMBER)} onClick={() => setW(x => ({ ...x, peak: !x.peak }))}>PEAK BURST: {w.peak ? "ON (+70 tps)" : "OFF"}</button>
          <button style={S.tog(w.countdowns, w.mode === 2, AMBER)} disabled={w.mode === 2} onClick={() => setW(x => ({ ...x, countdowns: !x.countdowns }))}>COUNTDOWN TASKS: {w.countdowns ? "ON (+25 held)" : "OFF"}</button>
          <button style={S.btn(false, w.mode === 2)} disabled={w.mode === 2} onClick={() => setW(x => ({ ...x, churn: 0, latency: 0, flow: false, outage: false, backlog: Math.round(x.backlog * 0.5), bounces: x.bounces + 1 }))}>BOUNCE THE BROKER ({w.bounces})<div style={{ color: "#8b90a0", fontSize: 10 }}>restart RabbitMQ to clear the jam - it comes back, then jams again at peak</div></button>
          <div style={{ ...S.label, marginTop: 12 }}>KAFKA'S OWN PHYSICS</div>
          <button style={S.tog(w.hol, w.mode !== 2, AMBER)} disabled={w.mode !== 2} onClick={() => setW(x => ({ ...x, hol: !x.hol, partDelay: 0 }))}>SLOW MESSAGE IN A PARTITION: {w.hol ? "ON" : "OFF"}</button>
          <button style={S.tog(w.nbWorker, w.mode !== 2, GREEN)} disabled={w.mode !== 2} onClick={() => setW(x => ({ ...x, nbWorker: !x.nbWorker, partDelay: 0 }))}>NON-BLOCKING WORKER: {w.nbWorker ? "ARMED" : "OFF"}<div style={{ color: "#8b90a0", fontSize: 10 }}>fetcher into a bounded local queue, then executors</div></button>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>DoorDash's 900+ async tasks - checkout included - ran on Celery and RabbitMQ. Under peak bursts the broker slowed down, and its own defenses made it worse: Flow Control throttled the app servers sending tasks, which they felt as network latency; harakiri kept killing slow workers, whose restarts flooded the broker with new connections; and HA mode cut throughput while its 20-plus-minute failovers jammed. When task processing went down, DoorDash went down.</div>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Push the peak at RabbitMQ and watch the queue push back - flow control, the kill-and-restart loop, the outage, the bounce. Switch on HA mode and reach the outage sooner. Then escape to Kafka, drain the same peak flat, and take on head-of-line blocking and rebalance stalls: the new problems you chose over the old ones.</div>
    </div>
  );
}
