...
const DT = 0.15, ENQ = 14, CAP = 100; // redis memory capacity units
...
export default function TheQueueThatCouldntDrain() {
  ...
  const verdict = (() => {
    if (!kafka && W.locked) return slow
      ? { c: RED, code: "SEIZED - FULL REDIS CANNOT DEQUEUE", t: "Memory is at the limit, and dequeuing needs a little free memory to move a job into the processing list, so the queue cannot empty itself. Enqueues are failing across every dependent operation. Fix the slowdown and watch: the lock does not release itself." }
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#aee6f8" : "#9aa0b0", background: on ? "rgba(54,197,240,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-07-31 review): the "break something" toggle carries its signal color (red)
    // permanently - ON/OFF shown by fill, never by brightness, so an off toggle stays
    // findable. The earlier off-state matched the page background and vanished.
    tog: (on) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: "pointer", border: `1px solid ${RED}`, color: RED, background: on ? "rgba(239,68,68,0.16)" : "#0c0d13", fontWeight: on ? 700 : 400, fontFamily: mono, fontSize: 11 }),
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 240px", minWidth: 240 }}>
          ...
          <button style={S.tog(slow)} onClick={() => setSlow(!slow)}>💥 DOWNSTREAM SLOWDOWN {slow ? "· ON" : "· OFF"}<div style={{ color: "#6b7080", fontSize: 10 }}>database contention slows job execution</div></button>
