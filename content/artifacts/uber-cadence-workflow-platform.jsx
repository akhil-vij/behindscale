...
const INJECTIONS = [
  ...
  { id: "engine", label: "ORCHESTRATOR / ENGINE HOST FAILS", hint: "the failure the embedded design avoids" },
  { id: "bug", label: "SHIP AN ENGINE FIX", hint: "who deploys, and how many times" },
  { id: "noisy", label: "NOISY SERVICE - 1M TIMERS FIRE AT ONCE", hint: "the failure that shapes Cadence's feature list" },
];

function verdicts(inj) {
  switch (inj) {
    case "worker": return {
      emb: { sev: "ok", code: "RECOVERS - REPLAY, LOCALLY", text: "The workflow resumes from its checkpoints inside the service. Committed steps are skipped, not re-executed: the guarantee the embedded library gives each service that carries it." },
      cen: { sev: "ok", code: "RECOVERS - REPLAY, CENTRALLY", text: "Cadence resumes the workflow from persisted state on another worker: durable and fault tolerant without the author thinking about it. Same guarantee, same crux answered: the only difference is where the engine lives. This is the durable-workflows pattern recurring across opposite architectures." },
    };
    case "engine": return {
      emb: { sev: "ok", code: "NO SHARED FATE TO LOSE", text: "There is no central engine host to fail. One service's engine dying is that service's incident; rides, payments, and the pipelines never notice. This is the exact shared-fate argument that leads some teams to reject a central orchestrator for their most critical services." },
      cen: { sev: "bad", code: "EVERY SERVICE STALLS TOGETHER", text: "One platform outage and every lane freezes - T0 rides and T5 training alike. All workflows stall until failover completes. Uber's answer is to industrialize exactly this risk: region failovers, a 2022 reliability year, and a 99.9% availability guarantee that is now the ceiling every service inherits." },
    };
    case "bug": return {
      ...
      cen: { sev: "ok", code: "ONE ROLLOUT - WITH A CATCH", text: "The platform team ships once and every service is patched. The catch is in-flight workflows: replay demands determinism, so the fix must be versioned against workflows already running - and six-plus years of backward compatibility means API mistakes are worked around, never removed. One rollout, carried carefully." },
    };
    case "noisy": return {
      emb: { sev: "ok", code: "ISOLATED BY CONSTRUCTION", text: "Your engine runs only your workflows. A service cannot be noisy in an architecture with no shared tenancy - the isolation Cadence must build and operate, the embedded design gets for free. (Its price was paid elsewhere: in the thousand deployments.)" },
      cen: { sev: "warn", code: "CONTAINED - BY PERMANENT DISCIPLINE", text: "One service's million timers land on shared shards and queues. The platform absorbs it with the machinery its feature list exists for: built-in rate limits, hot-shard detection, per-domain scale tracking. The burst is contained; the discipline never ends - capacity itself is managed by rate limits, with the database the bottleneck under everything." },
    };
    default: return {
      ...
      cen: { sev: "ok", code: "HEALTHY - ONE PLATFORM, 1,000+ SERVICES", text: "One engine, operated by ~20 engineers, runs 12 billion executions and 270 billion actions a month at Uber - teams write 40% less code for the same functionality." },
    };
  }
}
...
export default function TwoAnswers() {
  ...
  const S = {
    ...
    btn: (on) => ({
      ...
      border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#a7f3e9" : "#8b90a0",
  ...
  return (
    <div style={S.root}>
      ...
      <div style={{ color: "#edeff3", fontSize: 16, margin: "4px 0 2px", fontWeight: 700 }}>Two answers to the same workflow crash</div>
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>One design embeds the workflow engine in every service; the other builds one shared platform for a thousand of them. Break both, the same way, at the same time.</p>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 220px", minWidth: 220 }}>
          ...
          <div style={{ fontSize: 10, color: "#8b90a0", lineHeight: 1.8, marginTop: 6 }}>
            ...
            shared blast radius: <span style={{ color: "#c8cdd8" }}>none vs 99.9% ceiling, all services</span><br />
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Solve it once, centrally: Cadence, one multi-service workflow platform where workflows are ordinary code in native languages - durable, versioned, replayable - serving 1,000+ services from T0 to T5. The opposite answer to the identical problem is an embedded engine: a library in every service, with no shared platform at all.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Fire each failure at both designs at once and read the two verdicts. Both survive a worker crash. Only the central design is hurt by a platform outage. An engine fix ships once centrally, but a thousand times when embedded. And only the central design has to police a noisy service.</div>
    </div>
  );
}
