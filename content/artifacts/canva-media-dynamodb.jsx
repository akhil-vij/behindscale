...
// Walls light as media (millions) crosses thresholds; temporary fixes raise specific thresholds.
function walls(w) {
  ...
  return {
    ...
    ebs: { lit: t(1600, 6400, w.runway.shard), name: "STORAGE VOLUME 16TB + I/O LATENCY", fix: "shard by ID" },
    ext3: { lit: t(1600, 6400, w.runway.shard), name: "2TB TABLE-FILE LIMIT (DISK FORMAT)", fix: "shard by ID" },
  };
}
...
export default function EveryCeilingAtOnce() {
  ...
  const verdict = (() => {
    if (w.stage === "retro") return { c: VIOLET, code: "THE EXIT'S BILL, ITEMIZED", t: "Five years on: MAU more than tripled, DynamoDB autoscaled through all of it at lower cost than the RDS it replaced, 25B+ media stored. The bill: schema changes now mean writing and rigorously testing parallel-scan migration code; ad-hoc SQL is gone (rebuilt as a change-feed to the warehouse); composite indexes (GSIs) are still stitched together by hand. And the timestamped candor: facing this today, they'd strongly consider hosted NewSQL, Spanner or CockroachDB. This answer was right for 2017, not for all time." };
    if (w.stage === "cutover" && w.cut && w.flagBack) return { c: AMBER, code: "SECONDS TO MYSQL AND BACK", t: "The flag flipped and reads are back on MySQL - the rollback the checklist priced at seconds, which is exactly why switching writes was survivable risk instead of a bet. Flip it back and press on." };
    if (w.stage === "cutover" && w.cut) return { c: GREEN, code: "CUT OVER IN SILENCE", t: "Writes now land on DynamoDB through new code holding the old contracts with all-or-nothing and conditional writes. No downtime, no errors, and median and p95 (slow-request) latency IMPROVED through the migration window. It reads as luck only if you skip the apparatus: both-implementation test matrices, a rehearsed checklist, a seconds-priced rollback flag. Let the YEARS PASS." };
    if (w.stage === "cutover") return { c: AMBER, code: "THE RISKIEST STEP WANTS A REHEARSAL", t: "Switching writes is named the riskiest part of the whole process - new service code must guarantee the previous implementation's contracts. The checklist exists; it gets rehearsed through development and staging before production gets to see it. REHEARSE, then SWITCH WRITES." };
    if (w.stage === "migrating" && w.last === "list" && w.repl < 100) return { c: AMBER, code: "LIST-BY-USER MUST WAIT FOR THE SCAN", t: `Replication is at ${w.repl}% - but this query doesn't name an ID, so it can't serve from DynamoDB until every media has replicated. The new database comes online one query shape at a time, not by percentage: reads for a single media already work, but queries that list many must wait for 100%.` };
    if (w.stage === "migrating" && w.dualRead === "found") return { c: AMBER, code: "COMPARED IN PRODUCTION, BUG CAUGHT EARLY", t: "The dual-read comparison (every read served by MySQL and checked against the DynamoDB version) just surfaced a replication bug that test environments never would have. That's the lesson printed as an imperative: the data in production is always more interesting. COMPARE again after the fix." };
    if (w.stage === "migrating" && w.repl >= 100 && w.ecReads) return { c: GREEN, code: "THE SCAN IS DONE - EVERY QUERY SHAPE SERVES", t: "100% replicated: list-by-user and every other ID-less query can now read from DynamoDB, dual-read checked like the rest. Slightly-stale reads are already off MySQL. PROCEED TO CUTOVER - the writes are all that remain." };
    if (w.stage === "migrating" && w.repl > 0) return { c: GREEN, code: "REPLICATED HOT-FIRST", t: `${w.repl}% replicated - the hot set first: content-free queue messages (created / updated / read, never the content) captured live, workers re-reading truth from the MySQL primary and writing DynamoDB, so reorder and retry cost nothing. The scan walks the archive most-recent-first, under backpressure, only while the live queue runs empty. Try LIST-BY-USER mid-flight; run the DUAL-READ COMPARE.` };
    ...
    if (lit >= 3) return { c: RED, code: "EVERY CEILING AT ONCE", t: `${mediaLabel} media and ${lit} walls lit simultaneously: ${Object.values(W).filter(x => x.lit).map(x => x.name.toLowerCase()).join("; ")}. Several belong to the rented host, not MySQL: storage-volume caps, table-file limits from the disk format. Temporary fixes buy time; only the MIGRATION removes the walls.` };
    if (w.last && ["json", "denorm", "shard"].includes(w.last) && lit === 0 && w.media >= 800) return { c: AMBER, code: "RUNWAY BOUGHT, WALLS DARK - FOR NOW", t: `The temporary fix worked: ${w.last === "json" ? "the hottest schema churn moved into a JSON column the service manages itself" : w.last === "shard" ? "ID-range sharding dodges the file limit and replication ceiling, and prices every non-ID query as a slow scatter-gather (asking every server)" : "denormalization relieved lock contention at the cost of relational hygiene"}. Keep GROWING and watch the thresholds come back.` };
    if (lit > 0) return { c: AMBER, code: "THE WALLS ARE LIGHTING IN FORMATION", t: `${mediaLabel} media. ${lit} ceiling${lit > 1 ? "s" : ""} lit. Temporary fixes can raise these limits - JSON COLUMN for the schema wall, SHARD BY ID for the file/replication walls - but each grow doubles the media, and doubled media finds raised thresholds fast.` };
    return { c: AMBER, code: "A THIN LAYER OVER RDS, GROWING", t: `${mediaLabel} media on MySQL/RDS: vertical scaling, then eventually consistent replica reads. Read-heavy, recency-skewed, rarely modified after create. Press GROW and watch which walls arrive first - and notice how many belong to the rented host, not to MySQL itself.` };
  })();
  ...
  const S = {
    ...
    btn: (on, dis, col) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? (col || ACCENT) : "#4a4f60"}`, color: on ? "#aef2f5" : "#8b90a0", background: on ? "rgba(0,196,204,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>You run the media service as it grows explosively: watch the limits get hit one by one, apply the temporary fixes, then move off the relational database without any user noticing.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.btn(w.runway.json, !mysql || w.runway.json)} disabled={!mysql || w.runway.json} onClick={D("json")}>TEMP FIX: METADATA → JSON COLUMN</button>
          <button style={S.btn(w.runway.denorm, !mysql || w.runway.denorm)} disabled={!mysql || w.runway.denorm} onClick={D("denorm")}>TEMP FIX: DENORMALIZE, DROP FKs</button>
          <button style={S.btn(w.runway.shard, !mysql || w.runway.shard)} disabled={!mysql || w.runway.shard} onClick={D("shard")}>TEMP FIX: SHARD BY ID RANGE{w.runway.shard ? " (scatter-gather tax active)" : ""}</button>
          ...
          <button style={S.btn(w.rehearsed, !cutov || w.rehearsed)} disabled={!cutov || w.rehearsed} onClick={D("rehearse")}>REHEARSE THE CHECKLIST (dev → staging)</button>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>Canva's media tables hit every MySQL ceiling at once (six-week schema migrations blocking releases, replication rate caps, storage-volume limits, a 2TB table-file limit from the disk format), several of them belonging to the rented host (RDS) rather than to MySQL itself, with the number of media approaching a billion and still doubling.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Buy time with honest temporary fixes, then leave the relational database entirely: a live migration to DynamoDB via content-free queue messages (a worker re-reads the truth from MySQL and writes DynamoDB, so reorder and retry are free), hot data first, dual-read compared in production, then switch over using a rehearsed checklist, with a flag that could undo it in seconds. Zero downtime; latency improved.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Grow until the limits light up one after another; apply each temporary fix and watch the doubled media hit the raised limits again. Then run the migration in order: try the 'list a user's media' query mid-migration and see why some queries work before others, catch the bug the production comparison finds, rehearse before you switch writes, and read the honest list of what the move cost at the end.</div>
    </div>
  );
}
