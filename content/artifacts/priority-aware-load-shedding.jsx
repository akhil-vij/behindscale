...
// Pattern artifact — Priority-Aware Load Shedding (the general mechanism).
// Sim logic is VERBATIM from the landing hero (_hero.jsx, owner-edited); the
// only changes are (1) the token-alignment pass applied at port time —
// structural grays collapsed onto the named artifact tokens (--art-bg #08090D,
// --art-surface #0F1118, --art-surface-2 #161922, --art-border #1F2333,
// --art-text #C8CDD8, --art-text-muted #6B7280); GOLD/RED/GREEN and the batch
// BULK dot color stay as content accents — and (2) this comment. One source,
// two mounts: this bundle is the PALS pattern page's artifact AND (after the
// landing switch) the landing hero. See docs/pattern-artifacts-design.md §1.2.
...
export default function PatternShedding() {
  ...
  return (
    <div style={{ background: "#08090D", border: "1px solid #1F2333", borderRadius: 14,
      padding: 18, fontFamily: mono, color: "#C8CDD8", maxWidth: 640, margin: "0 auto" }}>

      {/* channel */}
      <div style={{ position: "relative", height: H, borderRadius: 10, overflow: "hidden",
        background: "linear-gradient(90deg,#08090D,#0F1118)", border: "1px solid #1F2333" }}>
        {/* source labels */}
        <div style={{ position: "absolute", left: 10, top: laneY(1) - 8, fontSize: 10, color: GOLD, letterSpacing: 1 }}>PAYMENTS</div>
        <div style={{ position: "absolute", left: 10, top: laneY(5) - 8, fontSize: 10, color: "#6B7280", letterSpacing: 1 }}>BATCH JOBS</div>

        {/* capacity gate line */}
        <div style={{ position: "absolute", left: `${GATE * 100}%`, top: 0, bottom: 0, width: 1,
          background: over ? `${RED}88` : "#1F2333" }} />
        <div style={{ position: "absolute", left: `${GATE * 100}%`, top: 6, transform: "translateX(-50%)",
          fontSize: 9, color: over ? RED : "#6B7280", letterSpacing: 1, whiteSpace: "nowrap" }}>
          CAPACITY {CAPACITY}/s
        </div>

        {/* server */}
        <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "10%",
          background: over ? `${RED}14` : `${GREEN}10`, borderLeft: `1px solid ${over ? RED + "55" : "#1F2333"}`,
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 14, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 180px", background: "#161922", border: `1px solid ${payPct >= 99 ? GREEN + "55" : RED + "55"}`,
          borderRadius: 10, padding: "10px 14px" }}>
          <div style={{ fontSize: 10, color: "#6B7280", letterSpacing: 1.5 }}>PAYMENTS SUCCEEDED</div>
          <div style={{ fontSize: 30, fontWeight: 700, color: payPct >= 99 ? GREEN : payPct > 70 ? GOLD : RED, lineHeight: 1.1 }}>
            {payPct}%
          </div>
        </div>
        <div style={{ flex: "1 1 130px", background: "#161922", border: "1px solid #1F2333", borderRadius: 10, padding: "10px 14px" }}>
          <div style={{ fontSize: 10, color: "#6B7280", letterSpacing: 1.5 }}>SERVED / OFFERED</div>
          <div style={{ fontSize: 30, fontWeight: 700, color: "#C8CDD8", lineHeight: 1.1 }}>
            {totalServed}<span style={{ fontSize: 15, color: "#6B7280" }}>/{offered}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 6, display: "flex", gap: 18, alignItems: "center", flexWrap: "wrap" }}>
        <label style={{ flex: "1 1 240px", fontSize: 11, color: "#6B7280", letterSpacing: 0.5 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, color: "#6B7280" }}>
            <span>TRAFFIC</span><span style={{ color: over ? RED : "#6B7280" }}>{offered} req/s</span>
          </div>
        ...
        <button onClick={() => setSmart((s) => !s)}
          style={{ padding: "9px 16px", borderRadius: 8, cursor: "pointer", fontFamily: mono, fontSize: 12,
            border: `1px solid ${smart ? GOLD : "#1F2333"}`,
            background: smart ? "rgba(245,184,65,0.12)" : "#0F1118",
            color: smart ? GOLD : "#6B7280", fontWeight: 600, letterSpacing: 0.5 }}>
