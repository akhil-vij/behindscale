...
export default function OnePodOnline() {
  ...
  const verdict = (() => {
    if (!podded) {
      if (w.dead !== null && w.ranAction) return { c: RED, code: "EVERY SHARD WAS EVERYONE'S PROBLEM", t: `Shard ${w.dead + 1} is down and the platform action (with_each_shard) is unavailable across the entire platform. Not just the shops on shard ${w.dead + 1}: everyone, because the action works only when all ${w.n} shards are up. Each shard you added was a new way to reach this verdict. Reorganize into PODS.` };
      if (w.dead !== null) return { c: AMBER, code: "ONE SHARD DOWN: NOW RUN THE PLATFORM ACTION", t: `Shard ${w.dead + 1}'s shops are down; everyone else is fine - so far. Run the PLATFORM ACTION and watch the one-line command tie every shop's fate to the one shard that's missing.` };
      return { c: AMBER, code: "SHARDED IN 2015: CAPACITY BOUGHT, RESILIENCE SOLD", t: `Vertical scaling ended; ${w.n} shards carry the shops. Watch the math on the right as you add shards: any action that touches all of them is up only when all of them are - availability ${(pAll * 100).toFixed(2)}% and falling with every shard. Kill one and run the platform action.` };
    }
    ...
    if (w.refused) return { c: VIOLET, code: "REFUSED: NO ACTION REACHES ACROSS PODS", t: "There is no platform-wide fan-out here; that is the whole point of the design. Platform-wide work (search across all shops, combined analytics) is rebuilt as slower background jobs that run outside the live request. The platform stopped having platform-wide failures by refusing to have platform-wide operations." };
    ...
    return { c: AMBER, code: "PODS: THE SERVERS WITHOUT THE SHARED FATE", t: `${w.n} pods, each a set of shops on fully isolated databases, with Sorting Hat assigning every request to exactly one. Adding a pod now adds capacity without adding a new way for the others to fail. Kill one and compare against the sharded era.` };
  })();
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#d6ecab" : "#8b90a0", background: on ? "rgba(132,204,22,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>The same set of machines, arranged two ways: as flat shards (2015) or grouped into isolated pods (2016). Add or remove them with the slider, kill one, and watch how far the damage spreads: across the whole platform, or stopped at a single pod.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <div style={{ color: "#6b7080", fontSize: 10 }}>{podded ? "each addition is pure capacity" : "each shard added makes every platform action likelier to fail"}</div>
        ...
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            <div style={S.label}>{podded ? "SORTING HAT → one pod per request · no cross-pod actions" : "SHARED APP TIER · click a shard below to kill it"}</div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
              <div style={{ flex: "1 1 150px", background: "#0c0d13", border: "1px solid #2a2a3a", borderRadius: 6, padding: "8px 10px" }}>
                ...
                <div style={{ fontSize: 9.5, color: "#6b7080" }}>{podded ? "no operation spans pods" : `= ${(U * 100).toFixed(1)}%^${w.n}, the product that falls as the fleet grows`}</div>
              </div>
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 8 }}><span style={lbl}>THE PROBLEM · </span>Shopify sharded MySQL in 2015 when vertical scaling ended - and the codebase's with_each_shard command meant any shard's failure took platform actions down everywhere. Availability dropped as more shards were added: every shard added was a new way to take the whole platform down.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Pods (2016): shops on fully isolated databases, shared compute allowed to speak to only one pod per action, and every unit of work assigned to exactly one pod - so serving a request needs one pod online. Sorting Hat routes requests to pods at the edge; Pod Mover evacuates a pod to its recovery DC in a minute, daily.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>In the sharded era, drag the fleet slider and watch the platform action's availability drop; kill a shard, run the platform action, and take everyone down. Flip to pods: the same kill stops at its walls, the platform action gets refused, and a dead data center is a one-minute evacuation.</div>
    </div>
  );
}
