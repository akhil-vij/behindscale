...
export default function DarkDashboards() {
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#9fdcff" : "#9aa0b0", background: on ? "rgba(0,162,255,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-07-31 review): INDEPENDENT MONITORING is the switch the whole artifact pivots on,
    // so it carries its signal color permanently - state shown by fill, never brightness, so an
    // off toggle stays findable instead of matching the dark background.
    tog: (on) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: "pointer", border: `1px solid ${ACCENT}`, color: ACCENT, background: on ? "rgba(0,162,255,0.16)" : "#0c0d13", fontWeight: on ? 700 : 400, fontFamily: mono, fontSize: 11 }),
  };
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.tog(indep)} onClick={() => setIndep(!indep)}>INDEPENDENT MONITORING: {indep ? "ON" : "OFF"}<div style={{ color: "#6b7080", fontSize: 10 }}>the post-outage remediation - observers off Consul</div></button>
        ...
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            ...
            {dashDark && <div style={{ fontSize: 10, color: RED, marginTop: 8 }}>▲ monitoring shares fate with the observed - the instruments went down with the patient</div>}
          </div>
        </div>
      </div>

      <div style={{ color: "#6b7080", fontSize: 10, marginTop: 12, borderTop: "1px solid #2a2a3a", paddingTop: 8, lineHeight: 1.7 }}>
        Hour costs are illustrative pacing; the sequence and mechanics are from Roblox's postmortem: four diagnosis attempts (hardware, 128-core NUMA machines that made contention worse, a snapshot reset that later seeded stale KV scheduling data, usage scaled to single digits) with monitoring that relied on Consul - Roblox states diagnosis challenges were largely responsible for the extended downtime and the monitoring dependency severely hampered triage; the streaming feature's Go-channel contention found via perf reports and flame graphs (KV p50 2s→300ms on disabling); slow leaders barred from election, explained days later as BoltDB writing a 7.8MB freelist per 16kB Raft append (4.2GB file, 489MB of data); cold-cache recovery of a 1B-req/s tier; and DNS-steered readmission in ~10% increments to 100% at hour 73. The independent-monitoring toggle is Roblox's own first remediation, made playable.
...
function ContextBlock() {
  ...
  return (
    <div style={{ background: "#111118", border: "1px solid #2a2a3a", borderRadius: 8, padding: "12px 14px", marginTop: 12 }}>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>THE MOVE · </span>Found via flame graphs: disable the streaming feature (contention on one Go channel), bar the BoltDB-crippled leaders from election, rebuild the cache tier cold, and readmit fifty million players 10% at a time. First remediation shipped: monitoring no longer depends on what it monitors.</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 6 }}><span style={lbl}>TRY · </span>Work the four theories with dark dashboards and watch the clock. Then flip INDEPENDENT MONITORING on, see the evidence light up, and compare the hours. Finish the recovery - gently, or open the floodgates and learn why not.</div>
    </div>
  );
}
