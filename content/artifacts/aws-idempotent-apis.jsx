...
export default function AskedTwiceDoneOnce() {
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#ffd9a8" : "#9aa0b0", background: on ? "rgba(255,153,0,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
    // FIX (2026-07-31 review): the three dedup-mode selectors are the choice the whole artifact
    // pivots on, so the chosen one carries the accent permanently and shows state by fill, never
    // brightness, so an unchosen mode stays findable instead of matching the dark background.
    tog: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.45 : 1, border: `1px solid ${ACCENT}`, color: on ? "#ffd9a8" : ACCENT, background: on ? "rgba(255,153,0,0.16)" : "#0c0d13", fontWeight: on ? 700 : 400, fontFamily: mono, fontSize: 11 }),
  };
    ...
    <div style={S.root}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.tog(w.mode === "none", !idle)} disabled={!idle} onClick={() => setW(x => ({ ...initial(), mode: "none" }))}>NO CONTRACT<div style={{ color: "#6b7080", fontSize: 10 }}>retries are just… more requests</div></button>
          <button style={S.tog(w.mode === "hash", !idle)} disabled={!idle} onClick={() => setW(x => ({ ...initial(), mode: "hash" }))}>SYNTHETIC HASH OF PARAMETERS<div style={{ color: "#6b7080", fontSize: 10 }}>identical-looking = duplicate (the tempting shortcut)</div></button>
          <button style={S.tog(w.mode === "token", !idle)} disabled={!idle} onClick={() => setW(x => ({ ...initial(), mode: "token" }))}>CLIENT TOKEN (caller-provided)<div style={{ color: "#6b7080", fontSize: 10 }}>intent declared: {TOKEN}</div></button>
