...
export default function ShardOrDoNotShard() {
  ...
  const verdict = (() => {
    if (w.era === "onebox" && w.load >= 100) return { c: RED, code: "THE CEILING IS ONE BOX TALL", t: `${w.pinsB}B Pins against a single-box engine - MySQL has no next gear here, and September 2011 has arrived: every piece of infrastructure over capacity at once. Two ways out: bet on a store that promises to scale itself, or keep the boring engine and do the distribution yourself.` };
    ...
    if (w.era === "cluster" && w.crashed) return { c: RED, code: "IT SCALED ITSELF UNTIL IT DIDN'T", t: "The cluster rebalanced itself and then broke catastrophically (the post's word), the way MongoDB, Cassandra, and Membase each broke on Pinterest in production. The automatic system you relied on failed in ways you couldn't see inside. The lesson writes itself: try really hard to just use MySQL. One way out left." };
    ...
    // sharded era
    if (w.splitDone) return { c: GREEN, code: "HALF THE SHARDS, TWICE THE HEADROOM", t: "Replicate MySQL001A to a fresh machine pair, flip one line of the shared config, and each machine now owns half the range. No object moved between shards - shards moved between machines, which is the only kind of move this system ever makes. That asymmetry is the whole placement contract." };
    ...
    if (w.lastId && !w.decomposed) return { c: GREEN, code: "FINDING DATA IS JUST MATH, NOT A LOOKUP", t: `Your Pin's ID composed live: shard ${w.lastId.shard}, type ${w.lastId.type}, row ${w.lastId.local}, packed into one 64-bit ID. Any client anywhere can now find this Pin with a couple of bit shifts, with no directory service and no hash ring. And it can never live anywhere else. Try decomposing the post's real Pin, or run the board join.` };
  ...
  const S = {
    ...
    btn: (on, dis) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 9px", marginTop: 6, borderRadius: 6, cursor: dis ? "not-allowed" : "pointer", opacity: dis ? 0.4 : 1, border: `1px solid ${on ? ACCENT : "#4a4f60"}`, color: on ? "#ffb3bd" : "#8b90a0", background: on ? "rgba(230,0,35,0.10)" : "#0c0d13", fontFamily: mono, fontSize: 11 }),
  };
  ...
  return (
    <div style={S.root}>
      ...
      <p style={{ color: "#8b90a0", fontSize: 11, margin: 0 }}>Grow through 2011 until one MySQL machine runs out of room, try both usual ways out, then shard by hand so finding any object is just simple math.</p>
      <ContextBlock />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
        <div style={{ ...S.panel, flex: "1 1 250px", minWidth: 250 }}>
          ...
          <button style={S.btn(w.era === "sharded", !((w.era === "onebox" && w.load >= 100) || w.crashed))} disabled={!((w.era === "onebox" && w.load >= 100) || w.crashed)} onClick={() => setW(x => ({ ...x, era: "sharded" }))}>SHARD BY HAND ON BORING MYSQL<div style={{ color: "#6b7080", fontSize: 10 }}>4,096 virtual shards, machine pairs</div></button>
        ...
        <div style={{ flex: "2 1 420px", minWidth: 300 }}>
          ...
          <div style={S.panel}>
            {w.era !== "sharded" ? (
              ...
              <div>
                <div style={S.label}>SHARD MAP (in ZooKeeper): ranges → machine pairs · production reads the PRIMARY only</div>
                {w.hosts.map(h => (
                ...
                {(w.lastId || w.decomposed) && (
                  <div style={{ marginTop: 12, background: "#0b0e12", border: "1px solid #2a2a3a", borderRadius: 6, padding: 10 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#8b90a0", marginTop: 6 }}>{w.decomposed ? `→ db0${d.shard}.pins, row ${d.local} - found with a couple of bit shifts, no lookup needed.` : "the ID packs shard, type, and row into one 64-bit number, and it can never change."}</div>
                  </div>
                )}
                {w.joinLog.length > 0 && (
                ...
                {w.alterAsked && (
                  <div style={{ marginTop: 10, fontSize: 11, color: GREEN }}>No schema change needed - teach the services the JSON has a new field, with a default for old blobs. (Pinterest: ~1 ALTER in 3 years.)</div>
                )}
              </div>
            )}
          </div>
